    ...
    for datas in data_arr:
        ...
        for i in range(len(listbei)):
            if data['content'].find(listbei[i])>0:
                countbei += 1
                #print(data['title'],listbei[i],countbei)
        countbei = 0
        ...
        sumbei = countbei + countai + countshang
        sumxi = countle + countxi + counthuan
        #laplace k = 5
        bei = (sumbei+5)/(sumbei+sumxi+5*2)
        xi = (sumxi+5)/(sumbei+sumxi+5*2)
        if bei != xi:
            print(data['title'],' ','悲：','percent: {:.2f}%'.format(bei*100),'喜：','percent: {:.2f}%'.format(xi*100))
        else:
            print(data['title'],'沉郁顿挫，乐景衬哀情')
